...
| **Face Recognition**      | Registration (real MobileFaceNet embedding model via `onnxruntime-node`, or client-submitted embeddings), cosine-similarity verification, liveness-gated                         |
...
Two features have one real external-service seam each that can't be exercised in this environment (no credentials for the real service) — everything else in those features is fully real. Face recognition's own seam changed shape rather than closing outright: registration now runs a real trained model, but with no face-detection/alignment step first (a different, narrower, honestly-documented gap than "no model at all"). See [Known Simplifications](#known-simplifications--future-work).
...
589 Jest tests across every module, none requiring a live database:
...
- **`faceEmbedding.provider.test.ts`** is the one exception to "fast, fully mocked" — it deliberately runs the real `onnxruntime-node` model end-to-end (see [Known Simplifications](#known-simplifications--future-work)) rather than mocking it, the same way `face.service.test.ts` mocks this exact module for everything that doesn't need to. It also runs under a custom Jest environment (`tests/utils/realGlobalsNodeEnv.js`, applied only to that one file via a `@jest-environment` docblock) — Jest's sandboxed test realm gives each file its own `Float32Array` constructor, and `onnxruntime-node`'s native tensor validation does a strict identity check against the real process's, which fails otherwise. A documented, Jest-maintainer-closed-as-not-planned limitation (`jestjs/jest#11864`), not a bug in this project's own code — confirmed by running the exact same logic successfully outside Jest first.
...
What this honestly still doesn't do: **detect or align the face first.** The whole uploaded photo is resized straight to the model's 112×112 input, not a cropped-and-aligned face region — InsightFace's own models are trained on aligned crops, so accuracy on an unaligned full photo is materially worse than a production pipeline that detects+aligns first. The same `buffalo_s` pack bundles a real face detector (`det_500m.onnx`, SCRFD) that was deliberately not added: decoding SCRFD's raw anchor-based output correctly needs real implementation work (anchor generation per stride, score thresholding, NMS) that this environment has no way to verify against real photos with known face locations — shipping unverified detection-decoding logic risked being worse than being explicit about the gap.

What this also can't verify: that the embeddings it produces are actually _discriminative_ for real human faces — this environment has no photos of real people with known identities to test recognition accuracy against. What **is** verified: this is the genuine, official InsightFace release (checksum-matched), it runs real inference (confirmed via `onnxruntime-node`, not a stub), and the output is deterministic and correctly shaped for every downstream consumer (`face.service.ts`'s cosine-similarity matching). `analytics.ai.service.ts`'s `duplicate_face` anomaly sweep now compares three possible embedding spaces (512-d real model, 67-d mobile's on-device geometric placeholder, and any 128-d vectors from registrations made before this model existed) — mismatched-length pairs are skipped rather than compared (`cosineSimilarity` correctly throws on a length mismatch; two different embedding spaces were never comparable in the first place), and each flagged pair's `detail` field says which kind of comparison it actually is.

Deployment note: `onnxruntime-node` ships no musl-compiled binary and fails outright on Alpine Linux (confirmed via [documented, maintainer-acknowledged issues](https://github.com/microsoft/onnxruntime/issues/2909) — not a guess). The `Dockerfile` was switched from `node:20-alpine` to `node:20-slim` across every stage for this reason, and its runtime stage now also copies `backend/models/` (which `tsc` never would on its own, since it isn't TypeScript). Unverified end-to-end: this environment has no Docker to actually build and run the image.
