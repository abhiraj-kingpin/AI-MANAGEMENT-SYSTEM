...
- **`faceRecognitionModels.test.ts`** is the exception to "fast, fully mocked" — it deliberately runs both real `onnxruntime-node` models (detector + embedding) end-to-end (see [Known Simplifications](#known-simplifications--future-work)) rather than mocking them, the same way `face.service.test.ts` mocks this exact module for everything that doesn't need to. It runs under a custom Jest environment (`tests/utils/realGlobalsNodeEnv.js`, applied via a `@jest-environment` docblock) — Jest's sandboxed test realm gives each file its own `Float32Array` constructor, and `onnxruntime-node`'s native tensor validation does a strict identity check against the real process's, which fails otherwise. A documented, Jest-maintainer-closed-as-not-planned limitation (`jestjs/jest#11864`), not a bug in this project's own code — confirmed by running the exact same logic successfully outside Jest first. The detector and embedding suites deliberately live in one file, not two: `onnxruntime-node`'s native "initialize once" guard is scoped to whatever copy of its own module a Jest test file happens to get, and Jest gives every file its own fresh module registry — two separate files each independently creating a real model session each called that native init as if for the first time, and the second call crashed the whole process (a hard native abort, not a catchable error; see the `v1.1.8` CHANGELOG entry for the full story). `faceAlign.test.ts` (the alignment math) needs neither a model nor this custom environment — it's pure geometry, verified with known-answer cases (a known scale, a known rotation, a known translation) instead.
