...
| **Face Recognition**      | Registration — real SCRFD detection + alignment + a real MobileFaceNet embedding model (all via `onnxruntime-node`), or client-submitted embeddings — cosine-similarity verification, liveness-gated |
...
608 Jest tests across every module, none requiring a live database:
...
- **`faceEmbedding.provider.test.ts` and `faceDetector.test.ts`** are the exception to "fast, fully mocked" — they deliberately run the real `onnxruntime-node` models end-to-end (see [Known Simplifications](#known-simplifications--future-work)) rather than mocking them, the same way `face.service.test.ts` mocks this exact module for everything that doesn't need to. Both run under a custom Jest environment (`tests/utils/realGlobalsNodeEnv.js`, applied only to those two files via a `@jest-environment` docblock) — Jest's sandboxed test realm gives each file its own `Float32Array` constructor, and `onnxruntime-node`'s native tensor validation does a strict identity check against the real process's, which fails otherwise. A documented, Jest-maintainer-closed-as-not-planned limitation (`jestjs/jest#11864`), not a bug in this project's own code — confirmed by running the exact same logic successfully outside Jest first. `faceAlign.test.ts` (the alignment math) needs neither a model nor this custom environment — it's pure geometry, verified with known-answer cases (a known scale, a known rotation, a known translation) instead.
