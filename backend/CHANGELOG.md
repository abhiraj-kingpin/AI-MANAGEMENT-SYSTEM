...
- **Dockerfile fixed for two real, verifiable-without-Docker deployment risks**: switched every stage from `node:20-alpine` to `node:20-slim` — `onnxruntime-node` ships no musl-compiled binary and fails outright on Alpine (confirmed via [documented upstream issues](https://github.com/microsoft/onnxruntime/issues/2909), not a guess); `sharp` and `bcryptjs` are unaffected either way. The runtime stage now also copies `backend/models/` into the final image, which it never did before (only `dist/` was copied, and `tsc` never touches non-TypeScript files). Neither fix is verified end-to-end — this environment has no Docker to actually build/run the image — but both are grounded in documented, external evidence, not speculation.
