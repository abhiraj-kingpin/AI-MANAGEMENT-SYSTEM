...
// const tuples (not `type` + array) so these can feed `z.enum(...)` directly
// in attendance.validators.ts — see shared/constants/roles.ts for the pattern.
...
/**
 * An employee-initiated request to amend their own check-in/out time —
 * distinct from `PATCH /attendance/:id/correct`, which is HR/Admin editing
 * a record directly with no approval step. Only the latest request is kept
 * (not a history array); a full audit trail lives in AuditLog regardless.
 * See docs/architecture/04-api-documentation.md#attendance-attendance.
 */
...
const attendanceSchema = new Schema<IAttendance>(
  {
    ...
    // Normalized to 00:00 in the office's local timezone by the service layer;
    // the {employeeId, date} unique index below is what actually enforces
    ...
    // Idempotency key for offline mobile writes — see
...
// Cross-field rules tying the verification `method` to the evidence it must
// carry — the schema-level guarantee behind
// docs/architecture/04-api-documentation.md's per-method check-in contract.
