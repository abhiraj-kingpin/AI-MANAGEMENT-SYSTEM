...
/**
 * The method-vs-evidence rule lives in a `pre('validate')` hook, which only
 * runs on the async `validate()` pipeline — `validateSync()` skips
 * pre/post hooks entirely, so these tests deliberately use `validate()`.
 * No DB connection is needed: nothing in the pipeline queries Mongo.
 */
