...
/**
 * A real integrity check, not documentation theater — reads
 * `backend/models/manifest.json` and verifies every listed model file's
 * *actual* SHA-256 and size on disk against what's recorded, and that no
 * undocumented `.onnx` file has been added to the directory without a
 * matching manifest entry. Minimal model governance: catches a model file
 * silently swapped, corrupted, or added without updating its provenance
 * record — a real regression class none of the other test files touch
 * (they only ever exercise a model's *behavior*, never its identity).
 */
