...
// approach as leaveCarryForward.service.test.ts.
...
// A known Monday — never a weekend, so tests aren't calendar-dependent.
...
describe('runAbsenceSweep', () => {
  ...
  it('marks only active employees with no Attendance record as absent', async () => {
    ...
    // emp-1 already has a record (checked in normally); emp-2 has none.
