...
/** Flushes the microtask queue so the fire-and-forget processRun() promise chain settles before assertions run. */
...
describe('payrollRunService.start / getStatus', () => {
  it('returns immediately with status "processing", without waiting for the batch to finish', async () => {
    ...
    // Never resolves during this test — proves start() doesn't block on the
    // batch completing (a real, deterministic check, not a microtask-count
    // guess about how much of the fire-and-forget loop happened to run
    // before this promise settled).
    ...
    expect(payrollRunService.getStatus(run.runId)).toBe(run); // same tracked record, not a copy
  });
