...
// Chosen so both hourlyRate (baseSalary/208) and dailyRate (baseSalary/31,
// August has 31 days) come out to clean integers — makes the expected
// numbers below exact, not floating-point approximations.
  ...
  it('computes grossPay/netPay/overtimePay/latePenalty from real attendance data', async () => {
    ...
    mockedAttendanceFind.mockReturnValue(
      mockQuery([
        ...
        attendanceRecord('present', 120), // 2h overtime on an otherwise-present day
      ]),
    );
    ...
    expect(mockedPayslipCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        employeeId: EMPLOYEE_ID,
        month: '2026-08',
        overtimePay: 93, // (120min/60) * hourlyRate(31) * 1.5
        latePenalty: 145.6, // dailyRate(208) * (2*0.1 + 1*0.5)
        grossPay: 8541, // 6448 base + 2000 allowances + 93 OT
        netPay: 6795.4, // 8541 - 1600 deductions - 145.6 penalty
        bonus: 0,
        status: 'generated',
      }),
    );
...
describe('payslipService.renderPdf', () => {
  it('produces a non-empty PDF buffer', async () => {
    ...
    expect(buffer.subarray(0, 4).toString()).toBe('%PDF'); // real PDF magic bytes, not a stub
  });
});
