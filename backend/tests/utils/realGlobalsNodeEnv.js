/* eslint-disable @typescript-eslint/no-require-imports, no-undef --
   Jest's `testEnvironment` docblock loads this file directly, outside the
   normal ts-jest transform pipeline — it must be plain CommonJS, not
   TypeScript, matching every example in Jest's own docs for a custom test
   environment. The only file in this backend that isn't. */
