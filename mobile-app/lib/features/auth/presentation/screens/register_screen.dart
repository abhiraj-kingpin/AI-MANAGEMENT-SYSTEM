...
/// Where a *new* employee actually gets in — not a full sign-up form.
/// HR has already added them (name, department, designation) in the admin
/// dashboard first; this screen only ever activates that existing,
/// not-yet-claimed record via `POST /auth/claim-account`, using the same
/// work email HR entered. There's deliberately no name/department/role
/// fields here — this can't create a new identity, only claim one that
/// already exists (see the backend's `auth.service.ts#claimAccount`).
