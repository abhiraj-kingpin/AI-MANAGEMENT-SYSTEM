...
## Backend v1.1.12 — Fix: Production Container Crashed on Startup

First real Render deploy after fixing the Root Directory setting built successfully but crashed on boot: a production-only log file transport tried to `mkdir logs/` as a non-root Docker user with no permission to create it. Fixed by dropping local file logging in production entirely — Render already captures stdout, which every log line was already going to regardless. Caught from an actual deploy log, verified by running the compiled server locally under `NODE_ENV=production` and confirming a clean boot. Full detail: [backend/CHANGELOG.md#v1112](backend/CHANGELOG.md).

## Admin Dashboard — "Cosmoq" Visual Redesign + New Landing Hero

Full restyle from a design handoff (`Cosmoq Dashboard Mockup.dc.html` + a written spec), implemented as real component/token changes rather than pasted HTML. Replaces the app's original single-blue "control room" theme with a warm-orange/cool-blue dual accent, and adds a one-shot animated landing hero shown before the dashboard on every fresh session.

**Tokens** (`src/index.css`): `Manrope`→`Inter`, `JetBrains Mono`→`Space Mono`; `--color-accent` `#2d5cff`→`#ff8a3d`; new `--color-accent-2` (`#3d8bff`) — the app is dual-accent now, not single-accent; glass surfaces/borders moved from blue-tinted to neutral (color now comes from what individual elements opt into on hover, not the base surface); new `--gradient-brand` (a plain CSS var, not a Tailwind color token, since it's a two-stop gradient) reserved for the handful of elements meant to carry brand identity itself.

**New:** `shared/ui/Logo.tsx` (a brain-and-circuit line icon on a `brand-gradient` badge, replacing the old plain "AI" text glyph), `shared/ui/icons.tsx` (inline SVG line-icons for every nav/topbar item, replacing Unicode glyphs — one deliberate deviation from the handoff's own reference icons: its "Notifications" glyph was a plain concentric circle, identical to Geofences' one ring smaller, read as a placeholder rather than a choice, so this reuses the real bell path already drawn for the topbar's own alert button instead), `features/landing/components/LandingHero.tsx` + the `hero → animating → dashboard` state machine now owned by `AppShell.tsx` (wheel/arrow-key/click triggered, one-shot per mount, nothing persisted — a hard reload shows it again by design).

**Restyled:** `Atmosphere` (two brand colors instead of one), `Sidebar` (brand-gradient active row + new icons + `Logo`), `Topbar` (brand-gradient avatar + SVG search/bell), `Button` (hover lift + glow, replacing the old diagonal shine-sweep), `StatCard` (per-metric hover glow — orange/blue/amber/slate — matching what each KPI represents rather than one shared accent), `AttendanceTrendChart` (its primary series now draws in on reveal along a brand-gradient stroke with a soft area fill and a glowing endpoint dot; the second real series — Late Rate, which the reference mockup's simplified single-line chart didn't have to account for — keeps its own plain line rather than being dropped to match the reference exactly).

One functional gap intentionally left alone: the topbar's search field is still a non-functional placeholder `<div>`, not a real `<input>` — the handoff described a focus-ring recolor for it, but there was never a real focused element to apply one to, and building actual search was out of scope for a restyle.

`tsc`/`eslint`/`prettier`/`vitest` (19/19) clean; production `vite build` clean; manually verified with a real backend + seeded local data (login → landing hero → transition → live dashboard), not just build success. Details: [admin-dashboard/README.md#design-system](admin-dashboard/README.md#design-system).
...
## Backend v1.1.11 — Minimal Model Governance: Model Cards + a Real Integrity Check

The last of six honestly-scoped ML gaps identified and closed this session, in order: real face detection/alignment (`v1.1.7`), real on-device mobile ML (see the mobile entry above), real accuracy measurement (`v1.1.9`), real anti-spoofing (`v1.1.9`), real unsupervised ML in analytics (`v1.1.10`), and now this — minimal model-governance documentation for all of it.

`backend/models/MODEL_CARDS.md` gives every real ML component (MobileFaceNet, SCRFD, MiniFASNet-V2, and the from-scratch Isolation Forest algorithm) a proper model card — purpose, provenance, license, checksum, input/output contract, what's actually been evaluated, and what honestly hasn't — consolidating facts already established across earlier entries in this history rather than inventing new claims. `backend/models/manifest.json` is the machine-readable counterpart, and unlike the markdown card it's actually checked: `tests/shared/modelManifest.test.ts` re-hashes every model file against it on every test run, catching a swapped, corrupted, or undocumented model file as a real, automatic test failure. Deliberately not built: a model registry, versioned artifact storage, or retraining pipeline — no real infrastructure exists here for any of that; "minimal" means real provenance and real integrity verification, not the full stack a production ML team would eventually want.

629 Jest tests (up from 623), `tsc`/`eslint`/`prettier` clean. Details: [backend/CHANGELOG.md#v1111--minimal-model-governance-model-cards--a-real-integrity-check](backend/CHANGELOG.md#v1111--minimal-model-governance-model-cards--a-real-integrity-check).

## Backend v1.1.10 — Real Unsupervised ML in Anomaly Detection (Isolation Forest)

Closed the last explicitly-named gap in "AI-Assisted Analytics": the anomaly-detection sweep had three honest rule-based checks and no actual trained model anywhere in it. `shared/ml/isolationForest.ts` implements Isolation Forest (Liu, Ting & Zhou, ICDM 2008 — the same algorithm behind scikit-learn's `IsolationForest`) from scratch, not a borrowed dependency, verified against known-outlier synthetic data. `detectAttendancePatternAnomalies` fits a fresh forest to the organization's own real attendance data on every request — mean/spread of check-in time, late rate, average overtime, scored jointly across all four dimensions — catching multi-dimensional outliers the existing per-dimension z-score check structurally can't. The other three anomaly checks deliberately stayed rules (an exact-threshold question like "is this GPS speed physically possible" is a genuinely better fit for a stated number than a model). Also fixed in passing: the admin dashboard's duplicate-face caveat text was stale, unconditionally claiming placeholder embeddings when real MobileFaceNet embeddings have been the common case since backend `v1.1.6`.

623 Jest tests (up from 614), `tsc`/`eslint`/`prettier` clean on both backend and admin-dashboard. Details: [backend/CHANGELOG.md#v1110--real-unsupervised-ml-in-anomaly-detection-isolation-forest](backend/CHANGELOG.md#v1110--real-unsupervised-ml-in-anomaly-detection-isolation-forest).
