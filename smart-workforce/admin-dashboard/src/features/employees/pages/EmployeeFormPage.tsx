...
function EmployeeFormBody({
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // The Department control is a custom listbox, not a native <select>, so
    // it doesn't get free HTML5 "required" validation from the browser —
    // check explicitly instead (mirrors how ApplyLeaveModal/AssignShiftModal
    // handle it by disabling Submit; this form's Submit isn't conditionally
    // disabled, so it needs the check here).
    if (!form.departmentId) {
      setFormError('Select a department before creating this employee.');
      return;
    }
    ...
    <div className="flex flex-col gap-6">
      ...
      <Reveal index={1}>
        <Card className="max-w-2xl p-6">
          {!isEdit && (
            <p className="mb-1.5 text-[10.5px] font-bold tracking-[0.06em] text-text-faint uppercase">
              Identity &amp; contact
            </p>
          )}
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {!isEdit && (
              <div className="sm:col-span-2">
                ...
                <p className="mt-1.5 text-[11.5px] font-medium text-text-faint">
                  This becomes their sign-in for the mobile app — they set a password on first
                  login.
                </p>
              </div>
            )}
            ...
            <p className="mb-1.5 text-[10.5px] font-bold tracking-[0.06em] text-text-faint uppercase sm:col-span-2">
              Role
            </p>
            ...
            <Field label="Department" htmlFor="departmentId">
              <Select
                id="departmentId"
                required
                disabled={departmentsLoading || (!departmentsLoading && departments?.length === 0)}
              ...
              {!departmentsLoading && departments?.length === 0 && (
                <p className="mt-1.5 text-[12px] text-warning">
                  ...
                  <Link to="/departments" className="font-semibold underline">
                    Add one
                  </Link>{' '}
                  first.
                </p>
              ...
              {departmentsError && (
                <p className="mt-1.5 text-[12px] text-danger">
                  Couldn't load departments. Try refreshing the page.
                </p>
              )}
            </Field>
