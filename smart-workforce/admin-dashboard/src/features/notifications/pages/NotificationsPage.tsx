...
export function NotificationsPage() {
  ...
  const visibleItems = (data?.items ?? []).filter((n) =>
    matchesQuery(searchQuery, n.title, n.body, TYPE_LABEL[n.type]),
  );
        ...
        <Card className="overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="rounded-pill bg-ink/5 px-2 py-0.5 text-[10.5px] font-bold text-text-dim uppercase">
          ...
          {searchQuery && data && data.pages > 1 && (
            <p className="border-t border-border px-5 py-3.5 text-[12px] text-text-faint">
              Search only matches notifications already loaded on this page — clear it to page
              through the rest.
            </p>
          )}
          {!searchQuery && data && data.pages > 1 && (
