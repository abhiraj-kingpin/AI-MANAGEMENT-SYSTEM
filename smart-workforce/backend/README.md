...
629 Jest tests across every module, none requiring a live database:
...
- **`tests/shared/*.test.ts`** — pure-function coverage for cross-cutting utilities (business-day math, shift-duration math, vector similarity, team scoping, actor helpers, the atomic sequence counter, regex-escaping) — including `isolationForest.test.ts` (`v1.1.10`), verified the same known-answer way: a real outlier planted in a synthetic cluster must score meaningfully higher than every inlier, not just "the code runs without throwing." `modelManifest.test.ts` (`v1.1.10`) is a different kind of test in this same directory — not testing a pure function's output, but reading `models/manifest.json` and asserting the real SHA-256/size of every model file on disk still matches what's recorded, catching a swapped, corrupted, or undocumented model file as a real, automatic failure (see [`models/MODEL_CARDS.md`](models/MODEL_CARDS.md)).
...
**Model governance**: every real ML model this backend loads at runtime — provenance, license, checksum, input/output contract, what's been evaluated and what hasn't — is documented in [`models/MODEL_CARDS.md`](models/MODEL_CARDS.md) (`v1.1.10`), the consolidated reference for the per-model detail scattered through this section below. Its checksums aren't just claims: [`models/manifest.json`](models/manifest.json) is verified against the real files on disk by [`tests/shared/modelManifest.test.ts`](tests/shared/modelManifest.test.ts) on every test run — a model file silently swapped, corrupted, or added without documentation is a real, automatically-caught test failure, not something that could go unnoticed.
