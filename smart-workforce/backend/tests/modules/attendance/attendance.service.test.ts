...
describe('attendanceService.checkIn — face', () => {
  ...
  it('checks in on livenessPassed: false — the mobile app is front-only now and no longer performs a liveness sequence, so this is no longer a rejection', async () => {
    mockedAttendanceFindOne.mockReturnValue(mockQuery(null));
    mockedFindNearestGeofence.mockResolvedValue(insideMatch);
    mockedFaceVerify.mockResolvedValue({ matched: true, confidence: 0.94 });
  ...
  it('checks in when livenessPassed is omitted entirely — identity (face match) and location (geofence) are the only required checks', async () => {
    mockedAttendanceFindOne.mockReturnValue(mockQuery(null));
    mockedFindNearestGeofence.mockResolvedValue(insideMatch);
    mockedFaceVerify.mockResolvedValue({ matched: true, confidence: 0.94 });
