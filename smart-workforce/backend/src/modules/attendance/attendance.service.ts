...
async function performCheckIn(
  ...
  const { method, location, qrToken, faceEmbedding } = input;
  ...
  if (method === 'face') {
    ...
    // The mobile app's face check-in is front-only now — no head-turn
    // liveness sequence to prove a live person vs. a held-up photo (a
    // deliberate simplification; livenessPassed is no longer required or
    // stored). Identity (face match) and location (geofence) below are the
    // anti-fraud checks this leans on instead — checked independently so
    // each failure gets its own honest error rather than one check
