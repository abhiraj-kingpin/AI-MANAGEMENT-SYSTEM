...
| [`admin-dashboard/`](admin-dashboard/) | Auth + live Dashboard KPIs + a full Employees CRUD screen (list/search/filter/paginate, create, edit, detail, deactivate). Remaining feature screens land alongside their own passes — see [admin-dashboard/README.md#features](admin-dashboard/README.md#features). | `lint`/`typecheck`/`format:check`/`build` clean, verified on GitHub Actions (`ci-admin.yml`)                                     |
...
- **Admin dashboard**: Employees (list/search/filter/create/edit/deactivate) is the first full feature screen; Attendance, Leave, Shifts, Payroll, Notifications, Geofences, QR, and the analytics trend-chart/department-comparison panel are still unbuilt.
