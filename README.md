...
| [`mobile-app/`](mobile-app/)           | Clean-Architecture scaffold + auth + GPS and Face check-in/check-out (real camera + on-device liveness; the embedding step is an honestly-documented placeholder — see [mobile-app/README.md#face-check-in](mobile-app/README.md#face-check-in)) + Face registration (same on-device embedding, submitted to the backend's new `POST /face/register-embeddings` — see [#face-registration](mobile-app/README.md#face-registration)) with attendance history and offline-queue fallback + self-service Leave (apply/cancel/balance) + Payslips (list + PDF download) + Notifications inbox. Hand-written without a Flutter SDK, machine-verified from Phase 20 onward once one became available. | `flutter analyze`: 0 issues; `flutter test`: 37/37 passing; `android`/`ios` scaffolded — see [mobile-app/README.md#status](mobile-app/README.md#status) |
