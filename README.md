...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [`backend/`](backend/)                 | **v1.1.6** — all 20 roadmap phases complete, plus a Departments CRUD API, three report-naming fixes (Attendance, Leave, Salary/Payslip), a second face-registration path (`POST /face/register-embeddings`), three previously-documented gaps closed (real leave carry-forward, absence-detection sweep, analytics PDF export), and face registration's photo→vector step now runs a real, trained MobileFaceNet model via `onnxruntime-node` (InsightFace's official `buffalo_s` pack, MIT license) instead of hashing image bytes — see [backend/README.md](backend/README.md) for exactly what that does and doesn't cover. | 589 Jest tests passing, `lint`/`typecheck`/`build` clean, verified green on GitHub Actions (not just locally), no live database required |
