...
| [`admin-dashboard/`](admin-dashboard/) | Auth + live Dashboard KPIs + Employees, Attendance, Leave, Shifts, Payroll, Notifications, Geofences, and QR Codes — every module from the original roadmap is now a real screen (see [admin-dashboard/README.md#features](admin-dashboard/README.md#features)). Building these surfaced and fixed four real backend gaps (Departments API, and employee-naming fixes for Attendance, Leave, and Salary/Payslip). Only the analytics trend-chart/department-comparison panel remains. | `lint`/`typecheck`/`format:check`/`build` clean, verified on GitHub Actions (`ci-admin.yml`)                                     |
...
- **Admin dashboard**: every module from the original roadmap is built — Employees, Attendance, Leave, Shifts, Payroll, Notifications, Geofences, and QR Codes (see [Component Status](#component-status)). Only the analytics trend-chart/department-comparison panel — a `DashboardPage` addition, not a separate screen — is still unbuilt.
