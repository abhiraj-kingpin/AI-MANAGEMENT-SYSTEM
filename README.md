...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [`backend/`](backend/)                 | **v1.1.5** — all 20 roadmap phases complete, plus a Departments CRUD API, three report-naming fixes (Attendance, Leave, Salary/Payslip), a second face-registration path (`POST /face/register-embeddings`), and three previously-documented gaps now closed: real leave carry-forward, a real absence-detection sweep, and an analytics PDF export. See [backend/README.md](backend/README.md) for the full feature list and API reference. | 588 Jest tests passing, `lint`/`typecheck`/`build` clean, verified green on GitHub Actions (not just locally), no live database required |
