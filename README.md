...
| [`mobile-app/`](mobile-app/)           | Clean-Architecture scaffold + auth vertical slice (login/logout/session-restore) + GPS check-in/check-out with attendance history + self-service Leave (apply/cancel/balance) + Payslips (list + PDF download) + Notifications inbox. Hand-written without a Flutter SDK, machine-verified from Phase 20 onward once one became available. | `flutter analyze`: 0 issues; `flutter test`: 16/16 passing; `android`/`ios` scaffolded — see [mobile-app/README.md#status](mobile-app/README.md#status) |
...
- **Mobile app**: auth, GPS check-in/check-out, self-service Leave, Payslips, and a Notifications inbox are built. QR/face check-in, shifts, and the offline sync half (a Hive local queue, connectivity listener, and retry logic against the now-real `POST /attendance/sync` API) are all still unbuilt — a scope gap now, not a tooling one, since Phase 20 got a real Flutter SDK verifying this codebase for the first time (see [mobile-app/README.md#status](mobile-app/README.md#status)).
