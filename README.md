...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [`backend/`](backend/)                 | **v1.1.1** — all 20 roadmap phases complete, plus a Departments CRUD API and an Attendance-report fix, both found while building the admin dashboard. See [backend/README.md](backend/README.md) for the full feature list and API reference. | 557 Jest tests passing, `lint`/`typecheck`/`build` clean, verified green on GitHub Actions (not just locally), no live database required |
| [`admin-dashboard/`](admin-dashboard/) | Auth + live Dashboard KPIs + Employees (full CRUD) + Attendance (report, filters, corrections). Building these two surfaced and fixed two real backend gaps (Departments API, Attendance employee names). Remaining feature screens land alongside their own passes — see [admin-dashboard/README.md#features](admin-dashboard/README.md#features). | `lint`/`typecheck`/`format:check`/`build` clean, verified on GitHub Actions (`ci-admin.yml`)                                     |
...
- **Admin dashboard**: Employees (list/search/filter/create/edit/deactivate) and Attendance (report, filters, corrections) are built; Leave, Shifts, Payroll, Notifications, Geofences, QR, and the analytics trend-chart/department-comparison panel are still unbuilt.
