...
- **Admin dashboard**: Employees (list/search/filter/create/edit/deactivate), Attendance (report, filters, corrections), Leave (self-service + review queue), and Shifts (self-service + definitions/assignment) are built; Payroll, Notifications, Geofences, QR, and the analytics trend-chart/department-comparison panel are still unbuilt.
