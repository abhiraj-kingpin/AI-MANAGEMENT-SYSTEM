...
| 8   | Face recognition                                                                                     | Partial — backend registration/verification done, now with two registration paths (photo upload, and `POST /face/register-embeddings` for client-computed embeddings); mobile check-in built (real camera + on-device liveness, placeholder embedding — see [mobile-app/README.md#face-check-in](mobile-app/README.md#face-check-in)); mobile *registration screen* not built yet, so nothing on the client currently calls the new embeddings endpoint — the two embedding methods can agree once it does, but don't yet in practice — see that section's Known Limitations |
