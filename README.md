...
| [`mobile-app/`](mobile-app/)           | **Feature-complete against the mobile scope of the original 20-phase roadmap.** Clean-Architecture scaffold + auth + GPS check-in/check-out with attendance history and offline-queue fallback + self-service Leave (apply/cancel/balance) + Payslips (list + PDF download) + Notifications inbox. Hand-written without a Flutter SDK, machine-verified from Phase 20 onward once one became available. | `flutter analyze`: 0 issues; `flutter test`: 20/20 passing; `android`/`ios` scaffolded — see [mobile-app/README.md#status](mobile-app/README.md#status) |
...
- **Mobile app**: feature-complete against the mobile scope of the original roadmap — auth, GPS check-in/check-out with offline-queue fallback, self-service Leave, Payslips, and a Notifications inbox are all built (see [mobile-app/README.md#status](mobile-app/README.md#status)). QR/face check-in and shifts have no dashboard/screen yet — both need a plugin this pass didn't add (camera/ML for face, a QR scanner) or, for shifts, simply haven't had their pass yet.
