...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [`backend/`](backend/)                 | **v1.1.8** — all 20 roadmap phases complete, plus a Departments CRUD API, three report-naming fixes (Attendance, Leave, Salary/Payslip), a second face-registration path (`POST /face/register-embeddings`), three previously-documented gaps closed (real leave carry-forward, absence-detection sweep, analytics PDF export), and face registration now runs a full real pipeline — SCRFD face detection, 5-point alignment, and a real MobileFaceNet embedding model, all via `onnxruntime-node` (InsightFace's official `buffalo_s` pack, MIT license) — instead of resizing a whole photo into a hash. See [backend/README.md](backend/README.md) for exactly what that does and doesn't cover. | 608 Jest tests passing, `lint`/`typecheck`/`build`/`docker-build` clean, verified green on GitHub Actions (not just locally — `v1.1.7`'s own first attempt at that claim was premature, see `v1.1.8`'s CHANGELOG entry), no live database required |
...
| 8   | Face recognition                                                                                     | Done — backend registration/verification (`POST /face/register`'s photo→vector step now real SCRFD detection + alignment + a real MobileFaceNet model, not a hash — see [Component Status](#component-status)), mobile check-in (real camera + on-device liveness + ⚠️ unverified real on-device embedding, same MobileFaceNet model as the backend, with a geometric fallback — see [mobile-app/README.md#face-check-in](mobile-app/README.md#face-check-in)) and mobile registration (same embedding step, `POST /face/register-embeddings`) all built and wired to each other — see [#face-registration](mobile-app/README.md#face-registration). One honest caveat beyond the unverified status: employees registered earlier via the original photo-upload path (or before the mobile embedding was real) need to re-register through the app once for their embeddings to actually match a check-in — see that section's Known Limitations |
...
- **Mobile app**: auth, GPS/QR/Face check-in/check-out with offline-queue fallback, Face registration, self-service Leave, Payslips, a Notifications inbox, and a read-only Shifts view are all built (see [mobile-app/README.md#status](mobile-app/README.md#status)). Face check-in's embedding step now runs the same real MobileFaceNet model the backend does (`FaceEmbeddingGenerator`, via ONNX Runtime's Flutter binding), not the geometric placeholder it started as — ⚠️ **unverified**: written with no Flutter/Dart SDK available in this environment, pending confirmation from `ci-mobile.yml`'s real Flutter toolchain (see [mobile-app/README.md#face-check-in](mobile-app/README.md#face-check-in)). The original geometric placeholder (`GeometricEmbeddingGenerator`) is kept as a same-device fallback if real inference can't initialize, decided once per capture attempt rather than per-frame, so one attempt is never a mix of the two incompatible vector spaces. Face registration submits mobile's embedding to the backend as-is, so newly-registered employees' check-ins actually match each other, and — new, also unverified — should now match backend photo-registrations too, since both run the identical model; employees registered before this existed still need one re-registration. See [mobile-app/README.md#known-limitations](mobile-app/README.md#known-limitations).
