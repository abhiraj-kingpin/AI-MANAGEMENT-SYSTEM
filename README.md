...
| [`admin-dashboard/`](admin-dashboard/) | Auth + live Dashboard KPIs + Employees (full CRUD) + Attendance (report, filters, corrections) + Leave (self-service + review queue) + Shifts (self-service "My Shift" + definitions/assignment) + Payroll (self-service payslips/PDF download + salary structures + payslip queue + run-payroll batch job) + Notifications (inbox, mark-read, Topbar unread badge, broadcast). Building these six surfaced and fixed four real backend gaps (Departments API, and employee-naming fixes for Attendance, Leave, and Salary/Payslip). Remaining feature screens land alongside their own passes — see [admin-dashboard/README.md#features](admin-dashboard/README.md#features). | `lint`/`typecheck`/`format:check`/`build` clean, verified on GitHub Actions (`ci-admin.yml`)                                     |
...
- **Admin dashboard**: Employees (list/search/filter/create/edit/deactivate), Attendance (report, filters, corrections), Leave (self-service + review queue), Shifts (self-service + definitions/assignment), Payroll (self-service payslips + salary/payslip management + run-payroll), and Notifications (inbox + broadcast) are built; Geofences, QR, and the analytics trend-chart/department-comparison panel are still unbuilt.
