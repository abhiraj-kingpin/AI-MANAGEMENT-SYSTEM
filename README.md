...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [`backend/`](backend/)                 | **v1.1.9** — all 20 roadmap phases complete, plus a Departments CRUD API, three report-naming fixes (Attendance, Leave, Salary/Payslip), a second face-registration path (`POST /face/register-embeddings`), three previously-documented gaps closed (real leave carry-forward, absence-detection sweep, analytics PDF export), face registration running a full real pipeline — SCRFD face detection, 5-point alignment, a real MobileFaceNet embedding model, and real MiniFASNet-V2 anti-spoofing, all via `onnxruntime-node` — instead of resizing a whole photo into a hash, and recognition accuracy now real-measured against LFW (96.97%, also catching and fixing a badly-miscalibrated match threshold). See [backend/README.md](backend/README.md) for exactly what that does and doesn't cover. | 614 Jest tests passing, `lint`/`typecheck`/`build`/`docker-build` clean, verified green on GitHub Actions, no live database required |
