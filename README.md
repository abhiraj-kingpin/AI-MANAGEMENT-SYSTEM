...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [`backend/`](backend/)                 | **v1.1.7** — all 20 roadmap phases complete, plus a Departments CRUD API, three report-naming fixes (Attendance, Leave, Salary/Payslip), a second face-registration path (`POST /face/register-embeddings`), three previously-documented gaps closed (real leave carry-forward, absence-detection sweep, analytics PDF export), and face registration now runs a full real pipeline — SCRFD face detection, 5-point alignment, and a real MobileFaceNet embedding model, all via `onnxruntime-node` (InsightFace's official `buffalo_s` pack, MIT license) — instead of resizing a whole photo into a hash. See [backend/README.md](backend/README.md) for exactly what that does and doesn't cover. | 608 Jest tests passing, `lint`/`typecheck`/`build` clean, verified green on GitHub Actions (not just locally), no live database required |
