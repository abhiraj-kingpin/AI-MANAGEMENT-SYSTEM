...
- `ci-mobile.yml` — `flutter analyze`, `flutter test`, and a debug `flutter build apk` on every push/PR touching `mobile-app/`. Originally shipped without the build step (the one-time `flutter create .` scaffolding hadn't been run yet, so `android`/`ios` didn't exist); both were added once a real Flutter SDK became available in Phase 20 to actually verify this codebase for the first time — see [mobile-app/README.md#status](../../mobile-app/README.md#status).
