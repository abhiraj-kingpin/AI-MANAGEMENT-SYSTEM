...
export const api = axios.create({
  ...
  // Sends the httpOnly refresh-token cookie on every request automatically —
  // see docs/architecture/07-authentication-flow.md.
...
// A separate, interceptor-free client for the refresh call itself, so a failed
...
async function refreshAccessToken(): Promise<string> {
  // De-dupe concurrent 401s (e.g. several widgets fetching at once) into one refresh call.
  if (!refreshPromise) {
