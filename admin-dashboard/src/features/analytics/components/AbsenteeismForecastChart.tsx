...
/**
 * A hand-rolled SVG line chart, same no-dependency approach as
 * `AttendanceTrendChart` — history as a solid line, the one forecast point
 * as a distinct dashed segment (vs. history's solid one) in the warning
 * color, so it never reads as a measured data point. Y-axis autoscales to
 * the data (unlike
 * `AttendanceTrendChart`'s fixed 0–100) since absenteeism rates cluster in
 * a much narrower band and a fixed 0–100 scale would flatten the line to
 * near-invisible.
 */
