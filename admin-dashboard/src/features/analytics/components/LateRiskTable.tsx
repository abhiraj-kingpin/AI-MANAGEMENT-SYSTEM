...
// Full literal class strings, deliberately — Tailwind's scanner needs the
// complete class name present in source somewhere to generate its CSS;
// building one via `bg-${tone}` template interpolation would silently
// produce no color at all for whichever tone never appears literally
// elsewhere in the codebase (`bg-warning`/`bg-success` don't, unlike
// `bg-danger`, which Topbar.tsx's notification badge already uses bare).
...
/** Ranked list, same "read top-to-bottom" shape as `DepartmentComparisonTable` rather than a full sortable data table — this is a triage view (who to look at first), not a report to slice and filter. */
            ...
            >
              ...
              <td className="px-5 py-3.5">
                <div className="flex items-center gap-2">
                  <div className="relative h-1.5 w-20 overflow-hidden rounded-pill bg-white/[0.06]">
                    <div
