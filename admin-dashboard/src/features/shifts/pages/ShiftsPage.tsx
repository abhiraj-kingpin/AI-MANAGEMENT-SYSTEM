...
import { useMyShift, useShifts } from '@/features/shifts/hooks/useShifts';
...
export function ShiftsPage() {
  ...
  const { data: myShift, isLoading: myShiftLoading } = useMyShift();
  const {
    data: shifts,
    ...
    <div className="flex flex-col gap-6">
      ...
      <Reveal index={1}>
        <Card className="px-5 py-4">
          <p className="text-[12.5px] font-bold text-text-dim">My Shift</p>
          {myShiftLoading ? (
            <p className="mt-1.5 text-sm text-text-dim">Loading…</p>
          ) : myShift ? (
            <>
              <p className="mt-1.5 text-lg font-extrabold">
                {myShift.shift.name}{' '}
                <span className="text-[13px] font-semibold text-text-dim">
                  {myShift.shift.startTime}–{myShift.shift.endTime}
                </span>
              </p>
              <p className="mt-1 text-[12px] text-text-dim">
                {TYPE_LABEL[myShift.shift.type]} · effective since{' '}
                {new Date(myShift.effectiveFrom).toLocaleDateString('en-US', {
                  month: 'short',
                  day: 'numeric',
                  year: 'numeric',
                })}
            ...
            <p className="mt-1.5 text-sm text-text-dim">No shift assigned yet.</p>
          )}
        </Card>
      </Reveal>
